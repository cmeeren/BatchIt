...
// Like testCaseAsync, but runs the test in parallel n times (useful for non-deterministic tests)
...
let tests =
    ...
    <| testList "Tests" [
        ...
        testList "waitMs" [
            ...
            parallelTestCaseAsync 1000 "Batches for waitMs after the first add"
            <| async {
                ...
                do! [ 1..8 ] |> List.map nonBatched |> Async.Parallel |> Async.Ignore

                do! Async.Sleep 50

                do! [ 9..16 ] |> List.map nonBatched |> Async.Parallel |> Async.Ignore
            ...
            parallelTestCaseAsync 1000 "Batches do not exceed maxSize even if arguments are added within waitMs"
            <| async {
                let mutable calledWith = []
        ...
        testList "unit return type" [
            ...
            parallelTestCaseAsync 1000 "overloads for unit return type work"
            <| async {
                let mutable calledWith = []
        ...
        testList "minWaitAfterAddMs/minWaitMs/maxWaitMs" [
            ...
            parallelTestCaseAsync 1000 "Extends batching by minWaitAfterAddMs after each add"
            <| async {
                let mutable calledWith = []
            ...
            parallelTestCaseAsync 1000 "Waits at least minWaitMs regardless of minWaitAfterAddMs"
            <| async {
                let mutable calledWith = []
            ...
            parallelTestCaseAsync 1000 "Waits at most maxWaitMs regardless of minWaitAfterAddMs"
            <| async {
                let mutable calledWith = []
            ...
            parallelTestCaseAsync
                1000
                "Batches do not exceed maxSize even if arguments are added within minWaitAfterAddMs/minWaitMs/maxWaitMs"
            <| async {
                let mutable calledWith = []
        ...
        testList "extra" [
            ...
            parallelTestCaseAsync 1000 "When sending extra, batches are split by extra value"
            <| async {
                let mutable calledWith = []
                ...
                do!
                    [ "a", 1; "a", 2; "a", 3; "a", 4; "other", 5; "a", 6; "a", 7; "a", 8 ]
